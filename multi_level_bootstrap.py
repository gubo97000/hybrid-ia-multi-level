...
def add_trace(
    ...
    if after_expl_R:  # Add result after explosion, normal join causes NaN
        ...
        joined_df = trace.join(
            R0_df
        )  # Join result with original nodes, work only if R0 comes from original G0
...
def get_bootstrap_G(G: nx.Graph, partition: dict):
...
def community_modularity(coms, g):
    ...
    for node in g:
        try:
            com = coms[node]
            deg[com] = deg.get(com, 0.0) + g.degree(node, weight="weight")
            for neighbor, dt in list(g[node].items()):
...
def get_worst_comm(trace, g, i, n_worst):
    coms = trace.iloc[:, i].to_dict()
    ...
    return [x[0] for x in sorted(res, key=lambda x: x[1])[:n_worst]]


def get_coms_mod_and_list(coms, g):
    if type(g) != nx.Graph:
        raise TypeError("Bad graph type, use only non directed graph")
    ...
    return sum([x[1] for x in res]), sorted(res, key=lambda x: x[1])
...
def fix_disconnected_comms(G: nx.Graph, trace: pd.DataFrame, i: int, offset):
    ...
    mod, coms_mod = get_coms_mod_and_list(fixed_res,G)
    # return res_trace, float(pq.PartitionQuality.community_modularity(fixed_res, G))
    return res_trace, float(mod), coms_mod
...
def hybrid_multi_level_beta(
    ...
    for i in range(max_levels):  # Each loop: G"i" graph -compute-> create G"i+1"
        ...
        trace, fixed_mod, coms_mod = fix_disconnected_comms(
            original_G0, trace, i, offset
        )
        ...
        # Update first best level index
        if best_1R != best_R:
            best_1R = best_R
            if verbose:
                print(f"!", sep="", end="")
            explode_pool = [x[0] for x in coms_mod[:int(len(coms_mod)/2)]] * try_close
        ...
        if len(fit_hist) > 1 and (fit_hist[-1] == fit_hist[-2]):
            ...
            if explosion:
                ...
                if verbose:
                    print(f"R{R_i} -> G{i+1} e{exploding_comm} ", end="")
                # display(trace)
                if try_close:
                ...
                print("-", end="")
                G1 = get_bootstrap_G(original_G0, after_expl_R)
                after_expl_R = {}
